...
impl Message {
    pub fn new_request(address: &str) -> Message {
        Message {
            header: Header {
                id: rand::random(),
                is_response: false,
...
impl Record {
    fn serialize(&self, buf: &mut Vec<u8>) {
        ...
        bytes::write_u16(buf, self.rdlength);
    ...
    fn deserialize(byte_reader: &mut bytes::ByteReader) -> Result<Record> {
        ...
        let rdlength = byte_reader.read_u16()?;
        let rdata = byte_reader.read_bytes(rdlength.into())?;
